...
def cascade_to_next(booking: Booking, current_offer: BookingOffer, db: Session) -> None:
    already_offered = db.query(BookingOffer.worker_id).filter_by(booking_id=booking.id).all()
    excluded_ids = {row.worker_id for row in already_offered}
    
    ranked_workers = get_ranked_workers(str(booking.skill), float(str(booking.lat)), float(str(booking.lng)), db)
