...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("role IN ('citizen', 'worker', 'admin')", name='ck_users_role')
    )
    ...
    op.create_table(
        ...
        sa.Column('availability', sa.Boolean(), server_default=sa.text('TRUE'), nullable=True),
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.CheckConstraint("status IN ('pending', 'assigned', 'completed', 'cancelled')", name='ck_bookings_status')
    )
    ...
    op.create_table(
        ...
        sa.Column('status', sa.String(length=20), server_default=sa.text("'offered'"), nullable=False),
        sa.Column('expires_at', sa.DateTime(timezone=True), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=True),
        sa.ForeignKeyConstraint(['booking_id'], ['bookings.id']),
        sa.ForeignKeyConstraint(['worker_id'], ['users.id']),
        sa.PrimaryKeyConstraint('id'),
        sa.CheckConstraint("status IN ('offered', 'accepted', 'declined', 'expired')", name='ck_booking_offers_status')
    )
