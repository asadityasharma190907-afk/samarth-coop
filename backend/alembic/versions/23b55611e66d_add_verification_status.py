...
down_revision: str | None = "c56d8283c0f0"
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE worker_profiles SET verified = TRUE WHERE verification_status = 'verified'"
    )
    op.execute(
        "UPDATE worker_profiles SET verified = FALSE WHERE verification_status != 'verified'"
    )
