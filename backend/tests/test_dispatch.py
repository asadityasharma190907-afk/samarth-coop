...
def test_compute_weekly_earnings_completed_bookings():
    ...
    try:
        citizen = User(
            name="Ravi", phone="9555555555", password_hash="hash", role="citizen"
        )
        worker = User(
            name="Suresh", phone="9111111111", password_hash="hash", role="worker"
        )
...
def test_get_ranked_workers_excludes_unverified_and_unavailable():
    ...
    try:
        ...
        w3_u = User(
            name="Valid Worker", phone="9333333333", password_hash="hash", role="worker"
        )
        db.add(w3_u)
        db.flush()
        w3_p = WorkerProfile(
