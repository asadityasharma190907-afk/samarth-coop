...
def test_demo_beat_e2e():
    ...
    suresh_id = get_user_id("9111111111")
    priya_id = get_user_id("9222222222")
    anil_id = get_user_id("9333333333")
    ...
    res = client.get("/booking-offers/worker", headers={"Authorization": f"Bearer {suresh_token}"})
    ...
    res = client.put(f"/booking-offers/{suresh_offer_id}", json={"action": "decline"}, headers={"Authorization": f"Bearer {suresh_token}"})
    assert res.status_code == 200, res.text
    
    # Step 4: Priya Gupta (rank 2) receives offer -> declines
    res = client.get("/booking-offers/worker", headers={"Authorization": f"Bearer {priya_token}"})
    ...
    res = client.put(f"/booking-offers/{priya_offer_id}", json={"action": "decline"}, headers={"Authorization": f"Bearer {priya_token}"})
    assert res.status_code == 200, res.text
    
    # Step 5: Anil Yadav (rank 3) receives offer -> declines
    res = client.get("/booking-offers/worker", headers={"Authorization": f"Bearer {anil_token}"})
    anil_offers = res.json()
    assert len(anil_offers) > 0, "Anil should have received the third offer"
    anil_offer_id = anil_offers[0]["id"]
    res = client.put(f"/booking-offers/{anil_offer_id}", json={"action": "decline"}, headers={"Authorization": f"Bearer {anil_token}"})
    assert res.status_code == 200, res.text
    
    # Step 6: Meena Verma (rank 4) receives offer -> accepts
    res = client.get("/booking-offers/worker", headers={"Authorization": f"Bearer {meena_token}"})
    ...
    res = client.put(f"/booking-offers/{meena_offer_id}", json={"action": "accept"}, headers={"Authorization": f"Bearer {meena_token}"})
    assert res.status_code == 200, res.text
    ...
    res = client.put(f"/bookings/{booking_id}/complete", headers={"Authorization": f"Bearer {meena_token}"})
    assert res.status_code == 200, res.text
    
    # Step 8: Ravi rates Meena 4 stars
    rating_payload = {"rating": 4, "review": "Great work"}
    res = client.post(f"/bookings/{booking_id}/rating", json=rating_payload, headers={"Authorization": f"Bearer {ravi_token}"})
    assert res.status_code == 200, res.text
    ...
    res = client.get(f"/booking-offers/booking/{booking_id}", headers={"Authorization": f"Bearer {admin_token}"})
    assert res.status_code == 200, res.text
    offers_audit = res.json()
    assert len(offers_audit) == 4
    
    # Step 10: Verify `/welfare-fund/summary`
    res = client.get("/welfare-fund/summary", headers={"Authorization": f"Bearer {admin_token}"})
    assert res.status_code == 200, res.text
    ...
    res = client.get(f"/wallet/{meena_id}", headers={"Authorization": f"Bearer {meena_token}"})
    assert res.status_code == 200, res.text
